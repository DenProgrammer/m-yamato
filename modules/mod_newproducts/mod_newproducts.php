...
if( !defined( '_VALID_MOS' ) && !defined( '_JEXEC' ) ) die( 'Direct Access to '.basename(__FILE__).' is not allowed.' );
	...
	$main = $rows[0];
	$image = (is_file($path.$main->product_full_image)) ? $path.$main->product_full_image : $defpath;
	$image1 = (is_file($path.$rows[1]->product_full_image)) ? $path.$rows[1]->product_full_image : $defpath;
	$image2 = (is_file($path.$rows[2]->product_full_image)) ? $path.$rows[2]->product_full_image : $defpath;
	$image3 = (is_file($path.$rows[3]->product_full_image)) ? $path.$rows[3]->product_full_image : $defpath;
