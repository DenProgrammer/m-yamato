			...
			if ($delimg) foreach($delimg as $k=>$v)
				{
					...
					if ($file)
						{
							$bfile = 'components/com_virtuemart/shop_image/product/'.$file->file_name;
							$sfile = 'components/com_virtuemart/shop_image/product/resized/'.$file->file_name;
					...
					if (is_file('components/com_virtuemart/shop_image/product/'.$prod->product_thumb_image))
						{
							...
							$bigimg = 'components/com_virtuemart/shop_image/product/'.$prod->product_full_image;
						}
					else
						{
							$bigimg = $img = 'components/com_virtuemart/themes/default/images/noimage.gif';
						}
	...
	foreach($avto as $a)	
		{
			...
			if (is_file('components/com_virtuemart/shop_image/product/'.$a->product_thumb_image))
				{
					$img = 'components/com_virtuemart/shop_image/product/'.$a->product_thumb_image;
				}
			else
				{
			...
			<div class="blockinfo">
				...
				<?php 
					echo '<a 
							...
							href="index.php?option=com_user&view=user&layout=etap&Itemid=24&id='.$a->product_id.'" 
						>'.$a->etap_dostavki.'</a>';
						if ($a->cdate) {
							echo ', '.date("d.m.Y", strtotime($a->cdate));
						}
				?>
			</div>
