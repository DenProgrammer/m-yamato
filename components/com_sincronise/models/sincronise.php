	...
	public function updateProduct($p_obj, $updateimg = false)
		{
			...
			if ($product) {
				...
				if (is_file('components/com_virtuemart/shop_image/product/'.$product->product_full_image)) {
					$product_image_hash = $product->image_hash;
				}
			} else {
				$metod = 'insert';
			}
			...
			if ((count($product_image)>0) and ($updateimg))
				{
					$i=0;
							...
							if (is_file($image))
								{
									...
									if ($product_image_hash != $image_hash) {
										$filename = $link.'.jpg';
										$thumb_filename = 'resized/'.$link.'.jpg';
										if (!$first_filename) $first_filename = $filename;
										if (!$first_thumb_filename) $first_thumb_filename = $thumb_filename;
										$img = getimagesize($image);
										$w = $img[0];
										$h = $img[1];
										
										if (($w > 800) or ($h > 600)){
											SincroniseModelSincronise::create_small($image,'components/com_virtuemart/shop_image/product/'.$filename,800, 600);
										} else {
											SincroniseModelSincronise::create_small($image,'components/com_virtuemart/shop_image/product/'.$filename,$w, $h);
										}
										if (($w > 280) or ($h > 200)){
											SincroniseModelSincronise::create_small($image,'components/com_virtuemart/shop_image/product/resized/'.$filename,280, 200);
										} else {
											SincroniseModelSincronise::create_small($image,'components/com_virtuemart/shop_image/product/resized/'.$filename,$w, $h);
										}
										
										$sql = "UPDATE `#__vm_product` SET
												`product_thumb_image`='$first_thumb_filename', 
												`product_full_image`='$first_filename', 
			...
			if ((count($product_image)>0) and ($product_id>0) and ($updateimg))
				{	
					$sep = $addwhere = '';
					foreach($product_image as $img){
						if (is_file($img)) {
							$addwhere .= $sep.'"'.md5(file_get_contents($img)).'"';
							$sep = ', ';
						} else {
							//echo 'file not found - '.$img.'<br>';
						}
					}
					...
					if ($addwhere) $sql .= ' AND `image_hash` NOT IN ('.$addwhere.')';
					$db->setQuery($sql);//echo $sql.'<br><br>';
					$db->query();
							...
							if (is_file($product_full_image))
								{
									...
									$file = file_get_contents($product_full_image);
									$image_hash = md5($file);
									
									$sql = 'SELECT `file_id` FROM `#__vm_product_files` WHERE `image_hash` = "'.$image_hash.'" AND file_product_id='.$product_id;
									$db->setQuery($sql);
									$check = $db->LoadResult();
									
									if (!$check){
										$filename = 'file_'.$product_id.'_'.$i.'.jpg';
										$thumb_filename = 'resized/file_'.$product_id.'_'.$i.'.jpg';
										if (!$first_filename) $first_filename = $filename;
										if (!$first_thumb_filename) $first_thumb_filename = $thumb_filename;
										$img = getimagesize($product_full_image);
										$w = $img[0];
										$h = $img[1];
										
										if (($w > 800) or ($h > 600)){
											SincroniseModelSincronise::create_small($product_full_image,'components/com_virtuemart/shop_image/product/'.$filename,800, 600);
										} else {
											SincroniseModelSincronise::create_small($product_full_image,'components/com_virtuemart/shop_image/product/'.$filename,$w, $h);
										}
										if (($w > 280) or ($h > 200)){
											SincroniseModelSincronise::create_small($product_full_image,'components/com_virtuemart/shop_image/product/resized/'.$filename,280, 200);
										} else {
											SincroniseModelSincronise::create_small($product_full_image,'components/com_virtuemart/shop_image/product/resized/'.$filename,$w, $h);
										}
										...
										$sql = "INSERT INTO `#__vm_product_files` (
													...
													'jpg', 'image/jpeg', 'components/com_virtuemart/shop_image/product/$filename', '1', 
													'1', '800', '600', '280', 
													'200', '1', '$image_hash')";
