...
{
	var $head;
	/*var $url = 'http://192.168.1.101/YaMoto/yamoto.1cws?wsdl';
	var $weburl = 'http://192.168.1.101/YaMoto/';
	var $user = 'user';
	var $pass = 'user';*/
	var $url = '';
	var $weburl = '';
	...
	//загузка автомобиля
	public function loadCars($id = null)
		{
			...
			foreach($xml->row as $k=>$item)
				{
					...
					$obj->cdate = strtotime(strval($item->Value[3]));
					$obj->product_s_desc = trim($item->Value[12].'');
					$images = $item->Value[25];
					$image = array();
					if ($images->row) foreach($images->row as $z)
						{
							$img = $z->Value[0];
							$img = str_replace('\\','/',(substr($img,strpos($img,'1c'))));
							$image[] = 'D:/xampp/htdocs/yamato.kg/'.$img;
						}
					$obj->product_image = $image;
					$obj->product_name = trim($item->Value[1].'');
					$obj->product_price = $item->Value[5].'';
					$obj->product_currency_link = $item->Value[6].'';
					$obj->product_currency = (isset($pcarr[$obj->product_currency_link])) ? $pcarr[$obj->product_currency_link] : '';
					$pub = (string) $item->Value[37];
					$obj->product_publish = ($pub == 'true') ? 'Y' : 'N';
					$obj->product_category_id = 1;
					$m = trim($item->Value[13]);//марка
					$obj->marka = (isset($mfarr[$m])) ? $mfarr[$m] : '';
					$obj->model = trim($item->Value[12].'');
					$obj->year = (int) $item->Value[4].'';
					$desc = trim(strval($item->Value[32]));
					$desc = str_replace("'", '"', $desc);
					$st = strpos($desc,'<body');
					$ed = strpos($desc,'</body');
					if (($st>0) and ($ed>0)) $obj->product_desc = substr($desc,$st+6,$ed-$st-6); else $obj->product_desc = $desc;
					$obj->RaspolojenieRulya = trim($item->Value[26].'');
					$obj->TipKPP = trim($item->Value[27].'');
					$obj->TipKuzova = trim($item->Value[28].'');
					$obj->EngineSize = trim($item->Value[7].'');
					$obj->NomerKuzov = trim($item->Value[29].'');
					$obj->Probeg = ($item->Value[30].'');
					$obj->TipTopliva = trim($item->Value[31].'');
					$obj->CvetSalona = trim($item->Value[34].'');
					$obj->TipSalona = trim($item->Value[34].'');
					$obj->AukcionnayaCena = trim($item->Value[38].'');
					$obj->ValutaAukcionnoiCeny = trim($item->Value[40].'');
					$obj->product_type = 'automobil';
					$PerezagruzitFoto = trim($item->Value[41].'');
					$obj->CenaProdaji = trim($item->Value[42].'');
					$obj->ValutaCenyProdaji = trim($item->Value[44].'');
					$obj->Dostavlen = trim($item->Value[45].'');
					$obj->product_in_stock = ($obj->Dostavlen == 'true') ? 1 : 0;
					$obj->DneiSMomentaPokupki = strval($item->Value[47]);
					$obj->status = strval($item->Value[48]);
					$obj->Sebestoimost = floatval($item->Value[50]);
					
					$updateimg = true;
					
					if (trim($item->Value[35].'') == 'true')
						{
							$obj->Prodan = 1;
							$obj->DataProdaji = strtotime(substr(trim($item->Value[36].''),0,strpos(trim($item->Value[36].''),'T')));
							$obj->product_in_stock = 0;
						}
					else
						{
			...
			foreach($xml->row as $k=>$item)
				{
					...
					$obj->TipPoSostavu = strval($item->Value[10]);
					$obj->TipPoNaznacheniyu = strval($item->Value[11]);
					$obj->Razmer = strval($item->Value[12]);
					$obj->group_uom = strval($item->Value[13]);
