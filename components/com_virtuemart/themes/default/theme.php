...
class vmTheme extends vmTemplate {
    ...
    function vmBuildFullImageLink($product) {
        ...
        /* Show the Thumbnail with a Link to the full IMAGE */ else {
    ...
    function vmlistAdditionalImages($product_id, $images, $title = '', $limit = 1000) {
        ...
        foreach ($images as $image) {
            ...
            if ($this->get_cfg('useLightBoxImages', 1)) {
                $html .= vmCommonHTML::getLightboxImageLink($mosConfig_live_site . '/' . $image->file_url, $thumbtag, $product_name, 'product' . $product_id);
            } else {
